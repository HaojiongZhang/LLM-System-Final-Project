 ...
 *
 * The extern "C" launcher receives d_kv_k and d_kv_v as DEVICE pointers
 * (already resident on GPU because the block pool is kept on-device in BlockManager).
 * Only queries, block_tables, seq_lengths, and the output buffer are transferred
 * host<->device inside this launcher.
 *
 * Build:
...
extern "C" void pagedAttention(
    float*       output,       /* host output   (B, n_head, head_dim)           */
    const float* queries,      /* host queries  (B, n_head, head_dim)           */
    const float* d_kv_k,      /* DEVICE ptr, single-layer slice of pool       */
    const float* d_kv_v,      /* DEVICE ptr, single-layer slice of pool       */
    const int*   block_tables, /* host  (B, max_blocks_per_seq)                 */
    const int*   seq_lengths,  /* host  (B,)                                    */
    int layer_idx,             /* unused (slice done on Python side), kept ABI  */
    int B, int n_head, int head_dim,
    ...
    /* d_kv_k and d_kv_v are already on device, so no copy is needed */
    ...
    cudaFree(d_sl);
    cudaFree(d_out);
    /* d_kv_k and d_kv_v are owned by BlockManager. Do not free here. */
}
