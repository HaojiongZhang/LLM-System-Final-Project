...
class CudaKernelOps(TensorOps):
    ...
    def paged_attention(
        ...
        kv_k_np,         # numpy (n_layers, n_blocks, n_head, block_size, head_dim)
        kv_v_np,         # numpy same shape
        ...
        Computes attention with online softmax directly over the block pool;
        ...
        kv_k_np         = np.ascontiguousarray(kv_k_np,         dtype=np.float32)
        kv_v_np         = np.ascontiguousarray(kv_v_np,         dtype=np.float32)
        ...
        n_layers, n_blocks = kv_k_np.shape[0], kv_k_np.shape[1]
        ...
        lib_paged.pagedAttention.argtypes = [
            ...
            np.ctypeslib.ndpointer(dtype=np.float32, flags="C_CONTIGUOUS"),  # kv_k
            np.ctypeslib.ndpointer(dtype=np.float32, flags="C_CONTIGUOUS"),  # kv_v
