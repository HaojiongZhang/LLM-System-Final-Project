...
def logsumexp(input: Tensor, dim: int) -> Tensor:
    ...
    """  
    ### BEGIN ASSIGN3_1
    m = Max.apply(input, tensor([dim]))
    return (input - m).exp().sum(dim=dim).log() + m
    ### END ASSIGN3_1
...
def softmax_loss(logits: Tensor, target: Tensor) -> Tensor:
    ...
    ### BEGIN ASSIGN3_1
    # Formula: ℓ(z, y) = log(∑exp(z_i)) - z_y
    # Use logsumexp for numerical stability
    lse = logsumexp(logits, dim=1)  # (batch_size, 1)
    target_one_hot = one_hot(target, logits.shape[1])  # (batch_size, C)
    target_logits = (logits * target_one_hot).sum(dim=1)  # (batch_size, 1)
