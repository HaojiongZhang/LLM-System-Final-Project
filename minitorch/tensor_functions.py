...
def flash_attention(
    ...
    if (
        q.f.flash_attention_forward is not None
        and q.f.flash_attention_backward is not None
    ):
    ...
    # Current integrated-attention keeps the FA2 backward path in
    # FlashAttentionFunc even when the backend lacks a dedicated flash forward.
    from .flash_attention_func import FlashAttentionFunc

    BoundFn = FlashAttentionFunc.make(causal=causal, scale=softmax_scale)
...
class FlashAttention(Function):
    ...
    def forward(
        ...
        assert q.f.flash_attention_forward is not None, (
            "Backend does not support flash attention forward"
        )
    ...
    def backward(ctx: Context, grad_output: Tensor):
        ...
        assert grad_output.f.flash_attention_backward is not None, (
            "Backend does not support flash attention backward"
        )
