...
\begin{abstract}
Large language model (LLM) inference is heavily bottlenecked by attention computation, which scales quadratically with sequence length and requires substantial memory for key-value (KV) cache storage. In this project, we explore two widely used system-level optimizations, FlashAttention and PagedAttention, and implement simplified versions within the MiniTorch framework. FlashAttention reduces memory movement during prefill, while PagedAttention improves memory efficiency during decoding. We evaluate these methods under controlled workloads and analyze their impact on latency, throughput, and memory usage. Our current results show two complementary trends. On the backward side, the FlashAttention-style kernel reduces allocated working memory by up to about $13\times$ while remaining near parity with a fair dense CUDA baseline at longer sequence lengths on a V100 GPU. On the forward side, our current tiled implementation is functionally competitive with the baseline but does not yet deliver a large end-to-end speedup because surrounding memcpy and allocation costs still dominate the measured runtime. We therefore view the current FlashAttention implementation as a correctness-focused, FA2-inspired educational system that already captures the intended memory-saving trend, but still lacks some of the systems-level optimizations needed for a consistent runtime win.
\end{abstract}
]
...
FlashAttention reorganizes attention computation to reduce high-bandwidth-memory traffic by tiling the score computation, maintaining online softmax statistics, and avoiding explicit materialization of the full $T \times T$ attention matrix \cite{dao2023flashattention2}. The key systems idea is that exact attention can be made substantially more memory-efficient if the kernel is scheduled around SRAM/shared-memory reuse rather than around large dense intermediates. FlashAttention-2 further improves this design by refining work partitioning, increasing parallelism across sequence tiles, and better matching the algorithm to GPU execution structure. These ideas motivate both the forward and backward kernels in our project: our implementation follows the same tiling-and-recomputation principle, while intentionally remaining simpler than production FA2 kernels so it can fit into the MiniTorch codebase and be studied more directly.

PagedAttention addresses a different bottleneck: during autoregressive decoding, the KV cache grows token by token and can become difficult to manage efficiently if each request requires a long contiguous allocation. The central idea is to decouple logical token order from physical storage by partitioning KV cache state into fixed-size blocks and maintaining a mapping layer between logical sequences and physical pages \cite{kwon2023pagedattention}. This design improves memory utilization, reduces fragmentation, and enables request scheduling strategies such as continuous batching that are critical in modern LLM serving systems. In our project, PagedAttention complements FlashAttention: FlashAttention targets prefill-time attention efficiency, while PagedAttention targets decode-time KV-cache efficiency and scheduler flexibility.

Taken together, these two lines of work frame our project as a systems study of exact attention rather than as a new attention approximation. Our contribution is to reproduce the main ideas of these methods inside MiniTorch, compare them against fair baselines, and analyze how much of the theoretical benefit survives in a compact educational implementation.
...
\begin{itemize}
    \item \textbf{Forward path}: computes attention outputs during prefill without materializing the full attention matrix.
    \item \textbf{Backward path}: computes gradients with a tiled CUDA kernel and is the main focus of this subsection.
\end{itemize}

Our forward path uses a \textbf{tiled, row-owned FlashAttention-style kernel}. One CUDA block owns one $(b,h,q\_\text{tile})$ region, and within that block each warp is assigned to one query row from the current tile while the block streams over key/value tiles. Dynamic shared memory is used to stage the active query tile together with each streamed key/value tile. Instead of writing the full attention matrix to global memory, the kernel maintains running row-wise statistics for the online softmax normalization and accumulates the output tile directly. On the host side, the launch configuration chooses tile sizes as a function of head dimension and GPU architecture so that larger key/value tiles can be used when shared-memory capacity allows better reuse. This design follows the same high-level FlashAttention principle as the backward path: reduce memory movement first, then reconstruct only the tile-local quantities needed for exact attention.
...
\paragraph{Scope note.} The forward and backward components are both implemented and summarized here, but the backward path remains the more detailed systems case study in this draft because its benchmark methodology and kernel analysis are more mature.
...
\paragraph{Forward workload note.} Our current forward evaluation uses the four smaller-to-medium configurations S1--S4 as end-to-end prefill workloads. These measurements are not directly comparable to the backward CUDA-event microbenchmarks: the forward results include surrounding runtime costs such as host/device copies and dynamic allocation, and we additionally use NSYS summaries to attribute time spent in memcpy and memory-management calls.

\paragraph{PagedAttention workload placeholder.} \textbf{[Add decode-length, request-mix, and KV-cache stress workloads here.]}

\section{Baselines}

At the full-project level, our report compares naive attention, FlashAttention, and PagedAttention, with primary emphasis on latency and GPU memory utilization.
...
\paragraph{Forward baseline note.} For the current forward study, the baseline is the existing dense attention forward path in MiniTorch. We compare it against the tiled FlashAttention forward implementation using end-to-end latency and NSYS-reported memcpy/allocation time. This forward comparison is therefore best interpreted as an application-level systems comparison rather than as a kernel-only microbenchmark.
...
\subsection{FlashAttention Forward}

Table~\ref{tab:flashforward_latency} summarizes our current forward-path measurements. Unlike the backward section, these numbers are \textbf{end-to-end runtimes} rather than CUDA-event kernel-only timings, so they include surrounding framework costs such as memory allocation and host/device transfer. Under this measurement methodology, the current tiled FlashAttention forward path is functionally competitive with the dense baseline, but its speedups remain small and inconsistent: it is slightly faster on the smallest and one medium configuration, essentially tied on the largest tested setting, and slightly slower on the remaining medium case.
...
\begin{tabular}{lccc}
\hline
Configuration & Baseline (s) & Flash (s) & Speedup \\
\hline
bs8, seq64, embd256, h8   & 0.027815 & 0.027469 & 1.033$\times$ \\
bs8, seq256, embd256, h8  & 0.208032 & 0.213609 & 0.974$\times$ \\
bs8, seq256, embd512, h8  & 0.222795 & 0.221005 & 1.008$\times$ \\
bs4, seq512, embd512, h8  & 0.476349 & 0.476250 & 1.000$\times$ \\
\hline
\end{tabular}
\caption{End-to-end latency comparison for the dense baseline and current FlashAttention forward implementation.}
\label{tab:flashforward_latency}
\end{table}

To understand why the forward speedups remain modest, Table~\ref{tab:flashforward_memory} reports NSYS summaries for transfer and allocation overhead. The most important pattern is that both implementations spend a large amount of time in \texttt{cudaMemcpy}, \texttt{cudaMalloc}, and \texttt{cudaFree}. In other words, even if the tiled forward kernel reduces attention-specific memory traffic, that gain is partially hidden by surrounding runtime costs that are shared by both implementations.
...
\begin{tabular}{lrrrr}
\hline
Config & Base memcpy & Flash memcpy & Base malloc & Flash malloc \\
& (ms) & (ms) & (ms) & (ms) \\
\hline
8,64,256,8   & 244.25  & 244.25  & 144.40 & 144.28 \\
8,256,256,8  & 2176.59 & 2172.18 & 279.79 & 277.23 \\
8,256,512,8  & 2380.16 & 2363.61 & 277.75 & 281.39 \\
4,512,512,8  & 4343.97 & 4344.35 & 309.17 & 300.52 \\
\hline
\end{tabular}
\caption{NSYS-reported transfer and allocation time for the dense baseline and FlashAttention forward implementation.}
\label{tab:flashforward_memory}
\end{table}

The NSYS profiles suggest that the benchmark is still dominated by \emph{shared surrounding costs} in both versions. In every configuration, large fractions of total time are attributed to \texttt{cudaMemcpy}, \texttt{cudaMalloc}, and \texttt{cudaFree}, together with substantial time in the same lower-level kernels such as matrix multiply, map, zip, and reduction operators. This means the current forward implementation is not yet reducing enough of the application-level runtime-critical work to create a clear end-to-end speedup, because the measured execution is still heavily influenced by transfer and allocation overhead outside the core attention kernel.

The memory-management data tell a similar story. FlashAttention forward does not yet consistently reduce observed allocation overhead at the application level. For example, the \texttt{cudaMalloc}/\texttt{cudaFree} totals remain very close between the baseline and FlashAttention runs across all four configurations, and the memcpy costs are nearly identical. As a result, the algorithmic advantage of the tiled kernel is largely masked by the fact that both paths still pay similar setup and transfer costs.

Overall, the current forward implementation should be interpreted as \textbf{algorithmically plausible but not yet systems-dominant}. It is already competitive with the baseline and preserves the intended FlashAttention design philosophy, but its end-to-end benefit is presently limited by repeated host/device copies, dynamic allocation, and the persistence of several unfused auxiliary operations around the core attention computation. The most important next steps are therefore to keep tensors resident on device across iterations, reuse temporary buffers, and route more of the forward path through a genuinely fused tiled implementation so that the algorithmic gains are visible at the application level.
