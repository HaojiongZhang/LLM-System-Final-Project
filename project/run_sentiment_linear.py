...
sor with same shape as y
    ones = minitorch.tensor_functions.ones(y.shape, backend=out.backend)
...
_dim
    ...
    def forward(self, embeddings):
        ...
        x = embeddings.mean(1)
        x = x.view(x.shape[0], int(x.size / x.shape[0]))
...
class SentenceSentimentTrain:
    ...
    def __init__(self, embedding_dim=50):
        self.model = Network(embedding_dim=embedding_dim)
...
if __name__ == "__main__":
    ...
    model_trainer = SentenceSentimentTrain(embedding_dim=embedding_dim)
    model_trainer.train(
        (X_train, y_train),
        learning_rate,
