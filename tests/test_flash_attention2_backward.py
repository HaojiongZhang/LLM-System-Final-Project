...
def _benchmark_case(causal: bool, bsz=2, nhead=3, seqlen=16, headdim=8, repeat=10):
    ...
    q_t = tensor_from_numpy(q)
    k_t = tensor_from_numpy(k)
    v_t = tensor_from_numpy(v)
    dout_t = tensor_from_numpy(dout)
    out_t = tensor_from_numpy(out)
    lse_t = tensor_from_numpy(lse)
    ...
    ref_best, ref_avg = _time_fn(ref_fn, warmup=1, repeat=repeat)
    flash_best, flash_avg = _time_fn(flash_fn, warmup=1, repeat=repeat)

    dq_ref, dk_ref, dv_ref = ref_fn()
    dq, dk, dv = flash_fn()
    ...
    print(
        f"[benchmark][causal={causal}] "
        f"shape=(B={bsz}, H={nhead}, T={seqlen}, D={headdim}) | "
        f"ref avg={ref_avg:.6f}s best={ref_best:.6f}s | "
        f"flash avg={flash_avg:.6f}s best={flash_best:.6f}s | "
        f"speedup(avg)={ref_avg / flash_avg:.3f}x | "
        f"speedup(best)={ref_best / flash_best:.3f}x"
    )
...
def test_flash_attention2_backward_benchmark_noncausal():
    _benchmark_case(causal=False, bsz=2, nhead=3, seqlen=16, headdim=8, repeat=10)


def test_flash_attention2_backward_benchmark_causal():
    _benchmark_case(causal=True, bsz=2, nhead=3, seqlen=16, headdim=8, repeat=10)
